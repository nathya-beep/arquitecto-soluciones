...
4. Compose the complete blueprint filling every section. The build order (§9) MUST be security-first and every endpoint/route step MUST state its auth + rate-limit + sensitive-data handling. The embedded CLAUDE.md (§15) MUST keep the "Building From the Blueprint" discipline block.
5. Write to `output/<project-name>-blueprint.md`
6. Present a summary to the user with the file path
7. Tell the user how to BUILD it: open a fresh Claude Code session inside the target project (with `BLUEPRINT.md` at its root) and paste the build prompt from `templates/build-prompt.md`. This is what enforces "read the plan first, build in order, don't skip security, confirm per block." Offer to output the ready-to-paste build prompt directly.
...
11. **Security by design, never bolted on.** Every blueprint MUST define authentication & registration, roles & permissions, endpoint/route protection (§8), rate limiting, and sensitive-data handling — what is encrypted, what is never stored, what is never exposed (§16). If the user hasn't given you enough to decide these, ASK before generating — do not invent security defaults silently.
12. **The build order (§9) must be security-first.** Arrange it so the security foundation (auth, session, protected-route middleware, roles, rate-limit utility) is built right after scaffolding + data model, never last. Every step that adds an endpoint/route/data field must state its auth check, required role, rate limit, and data-sensitivity handling in that same step.
13. **Always embed the "Building From the Blueprint" discipline** in the generated project's CLAUDE.md (§15): read the blueprint first, follow the build order block by block, don't skip §8/§16 security, summarize + confirm after each block, and stop-and-ask on any undefined security decision. Never omit this.
